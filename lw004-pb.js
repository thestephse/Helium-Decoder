...
function Decoder(bytes, port) {
  var packet_type = [
    ...
    "work_gps_fix_false",
    "work_ble_fix_success",
    "work_ble_fix_false",
    "helper_gps_fix_success",
    "helper_gps_fix_false",
    "helper_ble_fix_success",
    "helper_ble_fix_false",
  ];
  var dev_mode = [
    "off",
    "standby",
  ...
  if (port == 1) {
    ...
    if (lat > 0x80000000) lat = lat - 0x100000000;
    if (lon > 0x80000000) lon = lon - 0x100000000;

    dev_info.lat = lat / 10000000;
    dev_info.lon = lon / 10000000;
  } else if (port == 7 || port == 11) {
    var gps_fix_false_reason = [
      "hardware_error",
      "down_request_fix_interrupt",
      "mandown_fix_interrupt",
      "alarm_fix_interrupt",
      "gps_fix_tech_timeout",
      "gps_fix_timeout",
      "alert_short_time",
      "sos_short_time",
      "pdop_limit",
      "motion_start_interrupt",
      "motion_stop_interrupt",
    ];
...
function substringBytes(bytes, start, len) {
  ...
  for (var i = 0; i < len; i++) {
    char.push(
      "0x" + bytes[start + i].toString(16) < 0x10
        ? "0" + bytes[start + i].toString(16)
